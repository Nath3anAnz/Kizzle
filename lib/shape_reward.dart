...
class _ShapeRewardState extends State<ShapeReward> {
  ...
  void _playRewardSound() async {
    try { 
      await _sfxPlayer.play(AssetSource('audio/complete.mp3')); 
    } catch (e) {
  ...
  String get _kategoriName {
    if (widget.level == '1' || widget.level.toLowerCase() == 'easy') return 'Hewan';
    if (widget.level == '2' || widget.level.toLowerCase() == 'medium') return 'Perabotan';
    if (widget.level == '3' || widget.level.toLowerCase() == 'hard') return 'Bentuk';
    return 'Hebat';
  }
