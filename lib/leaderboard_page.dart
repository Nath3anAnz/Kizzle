...
class _LeaderboardPageState extends State<LeaderboardPage> {
  ...
  Widget _buildLeaderboardStream(String levelFilter) {
    return StreamBuilder<QuerySnapshot>(
      ...
      stream: FirebaseFirestore.instance
          ...
          .orderBy('waktu', descending: false) // Waktu paling sedikit (cepet) di atas
          .snapshots(),
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            int waktu = data['waktu'] is int ? data['waktu'] : int.tryParse(data['waktu'].toString()) ?? 0;
            int stars = data['stars'] is int ? data['stars'] : int.tryParse(data['stars'].toString()) ?? 0;
  ...
  Widget _buildPlayerCard(String name, int waktu, int stars, int rank) {
    ...
    if (rank == 1) {
      ...
      cardBorder = Border.all(color: Colors.amber, width: 2); // Bingkai emas buat juara 1
    } else if (rank == 2) {
